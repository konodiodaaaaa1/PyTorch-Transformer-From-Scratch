...
def translate_greedy(
    ...
    model.train()  # (!!!) 关键：如果之后还想继续训练，切回训练模式

    # 3. 解码
    if len(output_ids) > 1:
        return tgt_tokenizer.decode(output_ids[1:])
    else:
        return ""
...
class BeamSearchTranslator:
    ...
    def translate(self, src_sentence: str, beam_size: int = 5, max_len: int = 150,
        ...
        for i in range(max_len):
            ...
            last_word_log_probs[:, unk_column_index] += -10.0
    ...
    def translate_greedy(self, src_sentence: str, max_len: int = 150) -> str:
        """贪婪解码（用于debug）"""
        ...
        # 2. 自回归生成
        output_ids = [self.sos_idx]

        for i in range(max_len):
            tgt_tensor = torch.tensor([output_ids], dtype=torch.long, device=self.device)
            tgt_mask = create_look_ahead_mask(tgt_tensor, self.pad_idx).to(self.device)

            with torch.no_grad():
                dec_output = self.model.decoder(tgt_tensor, enc_output, tgt_mask, src_mask)
                logits = self.model.final_proj(dec_output)
            ...
            # DEBUG: 打印每一步
            word = self.tgt_tokenizer.decode([next_token])
            print(f"步骤{i + 1}: {word} (token_id={next_token})")
        ...
        # 3. 解码
        if len(output_ids) > 1:
            return self.tgt_tokenizer.decode(output_ids[1:])
        else:
            return ""
...
if __name__ == '__main__':
    ...
    # (替换为你保存的 pth 文件名)
    MODEL_FILE_NAME = "transformer_checkpoint_test.pth"  # <--- (确保文件名正确)
    ...
    while True:
        try:
            ...
            # --- (翻译) ---
            translation = translator.translate_greedy(src_sentence, MAX_LEN)
            # --- 打印结果 ---
            print("\n--- 翻译结果 (Beam Search) ---")
            print(translation)
            print("---------------------------------")
